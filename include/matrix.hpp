...
public:
    ...
    {
        // Go through all elements in the data
        for (auto &elem : data)
        {
    ...
    void multiply_partitioned(T factor, size_t n_partitions)
    {
        ...
        for (auto &t : slices)
        {
            // Make new thread which runs multiply_slice on this partition
            // std::ref sends objects as references, not copies
...
private:
    ...
    static void multiply_slice(T factor, typename std::vector<T>::iterator &begin, typename std::vector<T>::iterator &end)
    {
        ...
        {
            // it is an iterator, so *it is the element
            *it *= factor;
        }
    }
};
